...
sys.path.append('~/data/CGM/VBA/VBA-OMM/Python/VBM/')
import VBA_OMM
...
df_diet2['Insulin2'] = df_diet2['Insulin']/6.95
df_diet2[['copsacno','TIMEPOINT']] = df_diet2['ID'].str.split('_',expand=True)
df_diet2 = df_diet2[['copsacno', 'TIMEPOINT', 'Insulin2']]
...
test1 = df_glu_insulin[df_glu_insulin['copsacno'] == '95'].sort_values('TIMEPOINT')[['copsacno', 'TIMEPOINT', 'Glucose', 'Insulin2']]

test2 = df_glu_insulin[df_glu_insulin['copsacno'] == '101'].sort_values('TIMEPOINT')[['copsacno', 'TIMEPOINT', 'Glucose', 'Insulin2']]
...
def plot_indi(k, eatdot = True):
    ...
    orange_patch = mpatches.Patch(color='Orange', label='Eating (CGM after photo)')
    orange_line = Line2D([0], [0], label='Eating (Taking photo)', color='orange')
    if (eatdot):
        plt.legend(handles=[red_line, blue_patch, purple_patch, orange_patch], bbox_to_anchor=(1, 1), prop={'size': 15})
        #plt.legend(handles=[red_line, blue_patch, purple_patch, orange_patch], bbox_to_anchor=(1, 1.2), prop={'size': 15})
    else:
        plt.legend(handles=[red_line, blue_patch, purple_patch, orange_line], bbox_to_anchor=(1, 1), prop={'size': 15})
    plt.show()

k = 67
#Non_all_inf[k]

plot_indi(1, eatdot = True)
